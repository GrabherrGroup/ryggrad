...
void NeuralNetwork::MatchAndSort(svec<NNIO_WithCoords> & n)
{
...
int NeuralNetwork::Best(const NNIO & n)
{
  int index = 0;
  int i;
  double dist = -1.;
...
int NeuralNetwork::Retrieve(NNIO & n, double & score)
{
  int index = Best(n);
...
int NeuralNetwork::Retrieve(NNIO & n)
{
  int index = Best(n);
...
void NeuralNetwork::Learn(const NNIO & n, double ext_weight, bool bUpHit)
{
  int index = Best(n);
...
void NeuralNetwork::LearnAvoid(const NNIO & n, double weight)
{
  int index = Best(n);
  int i;
  double bestDist = n.Distance(m_neurons[index].Data());
...
void NeuralNetwork::LearnButPreserve(const NNIO & n, int iter)
{
  int i, j;
...
void TemporalNN::Retrieve(NNIO & out, const NNIO & in)
{
  out = in;
  m_nn[m_pointer].Retrieve(out);
}

void TemporalNN::Learn(const NNIO & n)
{
  m_nn[m_pointer].Learn(n);
}

void TemporalNN::LearnButPreserve(const NNIO & n)
{
