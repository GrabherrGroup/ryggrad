...
class Neuron
{
 public:
  ...
  void Update(const NNIO & n, double weight, int rot) {
    int i;
    while (rot < 0) {
      rot += m_data.isize();
    }
  ...
  void DeUpdate(const NNIO & n, double weight) {
    int i;
    for (i=0; i<m_data.isize(); i++) {
      //if (n.IsValid(i)) {
  ...
  void AsNNIO(NNIO & out, const NNIO & valid) const {
...
class NeuralNetwork
{
 public:
  ...
  void MatchAndSort(svec<NNIO_WithCoords> & n);

  double BestDist(const Neuron & n) const;
  int Best(const NNIO & n);
  int Retrieve(NNIO & n); // Returns the layer
  int Retrieve(NNIO & n, double & score); // Returns the index
  void Learn(const NNIO & n, double weight = 1., bool bUpHit = true);
  void LearnAvoid(const NNIO & n, double weight = 1.);

  // This addes new info while preserving old stuff
  void LearnButPreserve(const NNIO & n, int iter = 20);
...
class TemporalNN
{
 public:
  ...
  // Get the best match
  void Retrieve(NNIO & out, const NNIO & in);
  void Learn(const NNIO & n);

  // Learn from a single event, but don't destroy the memory
  // that's there already. 
  void LearnButPreserve(const NNIO & n);
